...
def check_and_update(target: str) -> dict:
    """
    ...
    if cycle_type == "day":
        ...
        if usage_date[:4] != now_date[:4]:
            flipped = True
            new_count = 1
    # forever: never flips, new_count stays usage_count + 1
...
def quota_check(params: list[str]) -> str:
    """Atomic check + consume one quota unit."""
    if not params:
        return json.dumps({"status": "error", "reason": "Missing target"})
    target = params[0]
    result = check_and_update(target)
