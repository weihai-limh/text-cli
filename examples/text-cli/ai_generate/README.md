...
Image and video generation via external AI APIs (CogView-3-Flash, CogVideoX-Flash compatible). API endpoints and model names are loaded from config, never hardcoded.
...
通过外部 AI API 生成图像和视频（兼容 CogView-3-Flash、CogVideoX-Flash）。API 端点和模型名从配置文件读取，不硬编码。
