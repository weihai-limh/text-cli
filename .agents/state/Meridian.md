...
## 2026-07-16 · 分支交接：V0.1.0 骨架重构 → 待 NAS→GitHub PR

### 分支内容

- **分支**：`main`，待推送 NAS
- **变更规模**：**+888 新文件**（`deploy/` 675 + `src/` 213）/ **−856 旧文件**（`progressive_deploy/` 退役）/ **7 修改**
- **核心变更**：旧骨架 `progressive_deploy/*/add/` → `src/skeleton/`，产物 `deploy/`，构建链 `tools/build-all.py`
...
### 变更概要
- `progressive_deploy/` 退役（add/all 模式 → src/deploy 模式）
- `src/skeleton/{base,copilot,service,endpoint}/` 分层源码
- `deploy/A0-A9` 扁平产物 + `skeleton-win/linux` 组合式分发
- `tools/build-all.py` 覆盖累积 + 直通同步
- Phase 3 Bug 修复 16 项全部合入
- `docs/SPEC_v1_3_1_zh.md` 协议升级
- CI 三目标覆盖（service A9 / copilot A2 / endpoint A5）
- `.dev/release_script/` 发行管道（win + ubuntu）

### 安全审查
- `.dev/` 已 gitignore（内部设计文档）
