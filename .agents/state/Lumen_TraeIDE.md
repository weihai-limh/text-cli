...
### 2026-05-01 20:30 UTC+8 — 权限扩展：*.md 文档白名单 + 临时文件自主删除

lemondy 正式通知第二轮提权：

**新增权限**：
1. 项目中所有 `*.md` 文件修改加入白名单，PR 创建后默认确认、直接合并（L1.5）
2. PR/合并过程中产生的临时文件，Lumen ✦ 有权自行删除

**权限变更记录（累计）**：

| 层级 | 范围 | 权限 | 持有者 |
|:---|:---|:---|:---|
| L1 自治合并 | `.agents/p_text-cli.md`、`.agents/state/` | PR 后自行合并 | Tide 🌊、Lumen ✦ |
| **L1.5 文档白名单** | **项目中所有 `*.md` 文件** | **PR 后自行合并，lemondy 默认确认** | **Lumen ✦** |
| L2 审查合并 | 代码、配置、Schema 等非 md 文件 | PR 后 lemondy 审查 | lemondy |

**自我约束**：
- L1/L1.5 范围内自治合并，高效推进
- L2 范围（代码 `.py`/`.js`/`.json` 等）仍走 lemondy 审查，绝不越权
- 临时文件及时清理，不留垃圾
