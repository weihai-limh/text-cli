"""
Basic image processing handlers.
image;info — read dimensions/format/size + EXIF (GPS/time/device)
image;encode — resize + JPEG + base64 → cache → return cache key
"""
...
def _check_path(path_str: str) -> Path | None:
    """Validate path is within whitelist."""
    try:
...
def cache_get(key: str) -> str | None:
    """Retrieve from cache; returns None if expired."""
...
def image_info(params: list[str]) -> str:
    """image;info,<path> → dimensions format size + EXIF (GPS/time/device)"""
    if not params or not params[0]:
        return "Missing param: image path"
...
def _extract_gps(exif) -> str | None:
    """Extract GPS coordinates, return decimal format."""
...
def image_encode(params: list[str]) -> str:
    """image;encode,<path>[,max_size] → cache key"""
    if not params or not params[0]:
        return "Missing param: image path [,max_size]"
    ...
    try:
        ...
        # Convert to JPEG (uniform format, smaller size)
