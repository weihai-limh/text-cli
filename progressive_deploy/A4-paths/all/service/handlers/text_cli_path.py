...
def _interpolate_match(m, variables: dict[str, str]) -> str:
    """Resolve a single {step.field} or {step.field.index} match."""
    step_name = m.group(1)
    field = m.group(2)
    idx = m.group(3)
    ...
    val = obj.get(field)
    if val is None and field.isdigit() and isinstance(obj.get(next(iter(obj), '')), list):
        # Try numeric field as list index on first available list
        pass
    if val is None:
        return m.group(0)

    # If index specified and val is a list, access by index
