...
— this script copies it as-is and bundles it for Linux distribution.
...
Usage:
    python scripts/release/ubuntu/build.py                      # default: A9
    python scripts/release/ubuntu/build.py --layer A3
    python scripts/release/ubuntu/build.py --version 0.2.0
    python scripts/release/ubuntu/build.py --layer A2 --version 0.2.0

Assembles deploy/skeleton-linux/text-cli-A{LAYER}-v{VER}/ from:
...
class LinuxReleaseBuilder:
    ...
    def _copy_packages(self):
        ...
        (base-js/) are excluded. start.sh sets TEXT_CLI_PACKAGE_SOURCE_DIRS
    ...
    def _generate_end_sh(self):
        """Generate end.sh for graceful service shutdown via fuser."""
...
""", encoding="utf-8")
    ...
    def _package_tar(self):
        ...
        Includes runtime directory AND sibling packages/ and protocol/ directories
        (if built).
        """
        import hashlib
        import tarfile
...
echo "========================================"
echo "  text-cli {layer} v{ver} - Linux"
echo "========================================"
echo ""
...
def main():
    ...
    builder = LinuxReleaseBuilder(version, args.layer)
    builder.run()
