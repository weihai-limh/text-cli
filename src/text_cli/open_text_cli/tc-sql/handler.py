...
def tc_sql_query(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-sql;query,<db_alias>,<JSON>"})
    ...
    try:
        ...
        if not auth_name:
            return json.dumps({"status": "error", "reason": "not authenticated"})

        db_perm = _resolve_db(auth_name, db_alias)

        table = query.get("table")
        if not table:
            return json.dumps({"status": "error", "reason": "missing 'table' in query JSON"})
        _check_table_op(db_perm, table, "query")

        columns = query.get("columns")
        if not columns or columns == "*":
            return json.dumps({"status": "error", "reason": "specify columns, SELECT * not allowed"})
...
def tc_sql_tables(params: list[str]) -> str:
    if not params:
        return json.dumps({"status": "error", "reason": "Usage: tc-sql;tables,<db_alias>"})
...
def tc_sql_schema(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-sql;schema,<db_alias>,<JSON>"})
    ...
    try:
        ...
        table = query.get("table")
        if not table:
            return json.dumps({"status": "error", "reason": "missing 'table' in schema JSON"})
...
def tc_sql_count(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-sql;count,<db_alias>,<JSON>"})
    ...
    try:
        ...
        table = query.get("table")
        if not table:
            return json.dumps({"status": "error", "reason": "missing 'table' in count JSON"})
