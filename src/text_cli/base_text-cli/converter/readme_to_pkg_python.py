...
def _generate_schema(domain: str, action: str, meta: dict, pkg_name: str) -> str:
    ...
    # Search keywords are positional; required params are shown as <...>
    # to align with the handler's documented usage (AI:domain;action,<entity>,<condition>).
    usage_parts = [f"<{p}>" for p in params]
    ...
    return json.dumps({
        ...
        "name_zh": pkg_name,
        "version": "0.1.0",
        "description": f"Knowledge base instruction package generated from '{pkg_name}'.",
        "description_zh": f"由 '{pkg_name}' 生成的知识库指令包。",
        "runtime": "python",
        "type": "native",
        "category": "knowledge",
        "locales": ["zh", "en"],
        ...
        "directives": [{
            ...
            "usage_zh": f"{domain_zh};{action_zh},{','.join(usage_parts)}" if usage_parts else f"{domain_zh};{action_zh}",
...
_TEMPLATE_PATH = Path(__file__).resolve().parent / "nocode_handler.py.tmpl"
_HANDLER_TMPL = _TEMPLATE_PATH.read_text(encoding="utf-8")
