...
def _load_st_prefixes():
    ...
    _registered_prefixes = {p.strip() for p in raw_reg.split(",") if p.strip()}
    raw_blk = os.getenv("ST_PREFIX_BLACKLIST", "")
    _blocked_prefixes = {p.strip() for p in raw_blk.split(",") if p.strip()}
...
def verify_access_token(token: str | None, required: bool = True) -> dict | None:
    ...
    clean = token
    clean = clean.removeprefix("Bearer ")
    clean = clean.strip()
