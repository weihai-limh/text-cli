...
def tc_table_read(params: list[str]) -> str:
    if not params:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;read,<file_path>[,<format>]"})
...
def tc_table_schema(params: list[str]) -> str:
    if not params:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;schema,<file_path>"})

    file_path = params[0]
    try:
        rows, _ = _read_file(file_path)
        if not rows:
            return json.dumps({"status": "ok", "source": file_path, "columns": [], "row_count": 0})
...
def tc_table_filter(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;filter,<json_data>,<JSON_query>"})

    rows = _parse_json_rows(params[0])
    if not rows:
        return json.dumps({"status": "error", "reason": "no data rows provided or JSON parse failed"})
    try:
        query = json.loads(params[1])
    except json.JSONDecodeError:
        return json.dumps({"status": "error", "reason": f"invalid JSON query: {params[1][:50]}"})

    where = query.get("where")
    if not where or not isinstance(where, list) or len(where) != 3:
        return json.dumps({"status": "error", "reason": "query must have 'where': [col, op, val]"})
...
def tc_table_sort(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;sort,<json_data>,<JSON_query>"})
    ...
    by = query.get("by")
    if not by:
        return json.dumps({"status": "error", "reason": "query must have 'by': column name(s)"})
...
def tc_table_pivot(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;pivot,<json_data>,<JSON_query>"})
    ...
    if not group_col:
        return json.dumps({"status": "error", "reason": "query must have 'group': column name"})

    if agg != "count" and not on_col:
        return json.dumps({"status": "error", "reason": f"agg='{agg}' requires 'on': column name"})
    ...
    for key, group_rows in groups.items():
        if agg == "count":
            ...
            if agg == "sum":
                ...
                return json.dumps({"status": "error", "reason": f"unknown agg: {agg}"})
...
def tc_table_join(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;join,<json_a>,<json_b>,<JSON_query>"})
    ...
    if len(params) > 2:
        ...
        try:
            ...
            return json.dumps({"status": "error", "reason": f"invalid JSON query: {query_str[:50]}"})

    if not left or not right:
        return json.dumps({"status": "error", "reason": "could not parse two JSON data sources"})

    join_type = query.get("type", "inner")
    on_col = query.get("on")

    if not on_col:
        return json.dumps({"status": "error", "reason": "query must have 'on': column name"})
...
def tc_table_write(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-table;write,<json_data>,<file_path>[,<format>]"})
    ...
    file_path = params[1]
    if not file_path:
        return json.dumps({"status": "error", "reason": "no file path provided"})
    ...
    try:
        ...
        if fmt == "xlsx":
            try:
                ...
                return json.dumps({"status": "error", "reason": "openpyxl required for .xlsx. pip install openpyxl"})
            wb = Workbook()
            ws = wb.active
            ws.append(col_names)
            for row in rows:
