...
def tc_diff_unified(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-diff;unified,<text_a>,<text_b>[,<context>,<label_a>,<label_b>]"})
...
def tc_diff_similarity(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-diff;similarity,<text_a>,<text_b>"})
...
def tc_diff_patch(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-diff;patch,<original>,<diff>"})
...
def tc_diff_word_diff(params: list[str]) -> str:
    if len(params) < 2:
        return json.dumps({"status": "error", "reason": "Usage: tc-diff;word-diff,<text_a>,<text_b>[,<format>]"})
    ...
    if fmt not in ("ops", "html"):
        return json.dumps({"status": "error", "reason": "format must be 'ops' or 'html'"})
