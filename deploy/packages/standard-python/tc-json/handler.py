...
def json_validate(params: list[str]) -> dict:
    if not params:
        return {"status": "error", "reason": "Usage: tc-json;validate,'<json>'"}
    try:
        json.loads(params[0])
...
def json_pretty(params: list[str]) -> dict:
    if not params:
        return {"status": "error", "reason": "Usage: tc-json;pretty,'<json>'"}
    try:
        obj = json.loads(params[0])
        return {"status": "ok", "result": json.dumps(obj, ensure_ascii=False, indent=2)}
    except json.JSONDecodeError as e:
...
def json_keys(params: list[str]) -> dict:
    if not params:
        return {"status": "error", "reason": "Usage: tc-json;keys,'<json>'"}
    try:
        obj = json.loads(params[0])
    except json.JSONDecodeError as e:
        return {"status": "error", "reason": f"Invalid JSON: {e}"}
    ...
    return {"status": "ok", "keys": list(obj.keys()), "count": len(obj)}



@directive("tc-json", "merge", domain_alias="tc-json", action_aliases={"merge": "合并"})
def json_merge(params: list[str]) -> dict:
    if len(params) < 2:
        return {"status": "error", "reason": "Usage: tc-json;merge,'<json1>','<json2>'"}
    try:
    ...
    if not isinstance(obj1, dict) or not isinstance(obj2, dict):
        return {"status": "error", "reason": "Both arguments must be JSON objects"}
    merged = {**obj1, **obj2}
...
def json_parse(params: list[str]) -> dict:
    if len(params) < 2:
        return {"status": "error", "reason": "Usage: tc-json;parse,'<json>','<dot-path>'"}
    try:
        obj = json.loads(params[0])
    except json.JSONDecodeError:
        return {"status": "error", "reason": "Input is not valid JSON"}
    result = _json_path_get(obj, params[1])
    if result is None:
        return {"status": "error", "reason": f"Path '{params[1]}' not found"}
...
def json_set(params: list[str]) -> dict:
    """tc-json;set,'<json>','<dot-path>','<value>' — set value at path."""
    if len(params) < 3:
        return {"status": "error", "reason": "Usage: tc-json;set,'<json>','<path>','<value>'"}
    try:
        obj = json.loads(params[0])
    except json.JSONDecodeError:
        return {"status": "error", "reason": "Input is not valid JSON"}
    if not isinstance(obj, dict):
        return {"status": "error", "reason": "Input must be a JSON object"}
    value = _try_parse_json_value(params[2])
    ok = _json_path_set(obj, params[1], value)
    if not ok:
        return {"status": "error", "reason": f"Cannot set path '{params[1]}'"}
    return {"status": "ok", "result": obj}



@directive("tc-json", "del", domain_alias="tc-json", action_aliases={"del": "删除"})
def json_del(params: list[str]) -> dict:
    """tc-json;del,'<json>','<dot-path>' — delete key at path."""
    if len(params) < 2:
        return {"status": "error", "reason": "Usage: tc-json;del,'<json>','<path>'"}
    try:
        obj = json.loads(params[0])
    except json.JSONDecodeError:
        return {"status": "error", "reason": "Input is not valid JSON"}
    ...
    if not ok:
        return {"status": "error", "reason": f"Path '{params[1]}' not found or not deletable"}
    return {"status": "ok", "result": obj}



@directive("tc-json", "pick", domain_alias="tc-json", action_aliases={"pick": "提取"})
def json_pick(params: list[str]) -> dict:
    """tc-json;pick,'<json>','<path1>[,<path2>,...]' — extract selected paths."""
    if len(params) < 2:
        return {"status": "error", "reason": "Usage: tc-json;pick,'<json>','<path1>[,<path2>,...]'"}
    try:
        obj = json.loads(params[0])
    except json.JSONDecodeError:
        return {"status": "error", "reason": "Input is not valid JSON"}
    ...
    try:
        ...
        return {"status": "error", "reason": "Second param must be a JSON array of dot-paths"}
    result = {}
    for path in paths:
        val = _json_path_get(obj, path)
        if val is not None:
            _json_path_set(result, path, val)
    return {"status": "ok", "result": result, "count": len(paths)}
...
def json_split(params: list[str]) -> dict:
    """tc-json;split,'<text>'[,<size>][,<overlap>] — text → JSONL file."""
    if not params:
        return {"status": "error", "reason": "Usage: tc-json;split,'<text>'[,<size>][,<overlap>]"}
...
def json_join(params: list[str]) -> dict:
    """tc-json;join,'<path>'[,<field>] — JSONL file → text file."""
    if not params:
        return {"status": "error", "reason": "Usage: tc-json;join,'<path>'[,<field>]"}

    src_path = Path(params[0])
    field = params[1] if len(params) > 1 and params[1] else "text"

    if not src_path.exists():
        return {"status": "error", "reason": f"File not found: {src_path}"}
    ...
    try:
        ...
        return {"status": "error", "reason": f"JSON parse error in file: {e}"}
