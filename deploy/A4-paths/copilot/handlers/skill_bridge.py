...
class SkillBridgeHandlers:
    ...
    def _try_skill_bridge(self, canonical: str, params: list) -> dict | None:
        ...
        # ── Whitelist gate for skill bridge (ISSUE-002) ──
        domain, action = canonical.split(";", 1) if ";" in canonical else ("", canonical)
        wl_entry = self.whitelist.lookup(domain, action)
        if wl_entry is not None:
            ...
            if wl_entry.get("_compiled") and not wl_entry["_compiled"].fullmatch(arg_str):
                return error("ACCESS_DENIED",
                             f"参数不符白名单 (skill bridge): {canonical} args={arg_str!r}")
