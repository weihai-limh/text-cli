...
| textcli-loader | pip | Python | `textcli_loader/` + `pyproject.toml` | pip 可安装的零依赖包加载器——任何 Python 环境直接加载和执��指令包 |

## 与 service 的差异

| 维度 | service（标准运行时） | cloudbase（云函数） | textcli-loader（pip 包） |
|------|------|------|------|
...
| 文件 | 说明 |
|------|------|
| `textcli_loader/parser.py` | 指令解析器（与 service core/parser.py 同构） |
| `textcli_loader/registry.py` | `@directive` 装饰器注册表 |
| `textcli_loader/loader.py` | schema.json + handler.py 动态加载 |
| `textcli_loader/envelope.py` | 统一信封格式（兼容 text-cli service） |
