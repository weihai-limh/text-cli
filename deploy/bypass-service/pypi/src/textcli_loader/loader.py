...
def _import_handler(pkg_id: str, handler_path: Path):
    ...
    # Inject compatibility shims so existing text-cli packages work
    # transparently — they may import from 'core.registry' (service runtime)
    # or 'textcli_loader.registry' (standalone loader).
    _reg = __import__("textcli_loader.registry", fromlist=["directive"])
    sys.modules.setdefault("textcli_loader.registry", _reg)
