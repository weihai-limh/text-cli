...
# Map copilot internal error codes to protocol closed set (SPEC §1.2.8)
_ERROR_CODE_MAP: dict[str, str] = {
    "unknown_instruction": "ERR_NOT_FOUND",
    "not_found": "ERR_NOT_FOUND",
    "internal_error": "ERR_EXECUTION",
    "parse_error": "INVALID_PARAMS",
    "missing_param": "INVALID_PARAMS",
    "bad_request": "INVALID_PARAMS",
    "unauthorized": "ACCESS_DENIED",
    "install_failed": "ERR_EXECUTION",
    "uninstall_failed": "ERR_EXECUTION",
    "skill_timeout": "ERR_EXECUTION",
    "skill_error": "ERR_EXECUTION",
    "skill_failed": "ERR_EXECUTION",
    "adapter_missing": "ERR_EXECUTION",
    "adapter_error": "ERR_EXECUTION",
    "output_adapter_error": "ERR_EXECUTION",
    "encode_error": "ERR_EXECUTION",
    "decode_error": "ERR_EXECUTION",
    "invalid_package": "INVALID_PARAMS",
    "invalid_schema": "INVALID_PARAMS",
    "already_installed": "ERR_EXECUTION",
    "not_installed": "ERR_EXECUTION",
}

_VALID_RST_TYPES = frozenset({"text", "picture", "video", "audio", "file"})
...
def ok(data: dict, rst_type: str = "text") -> dict:
    ...
    rst_type: response type, "text" by default. Must be one of:
              text / picture / video / audio / file (SPEC §1.2.2).
              If data contains pray_rst_types, that value takes precedence
              and is stripped from rst_data (only if it's a valid type).
    """
    if rst_type not in _VALID_RST_TYPES:
        rst_type = "text"
    if isinstance(data, dict):
        pray = data.pop("pray_rst_types", None)
        if pray and pray in _VALID_RST_TYPES and rst_type == "text":
            rst_type = pray
...
def error(code: str, detail: str, **extra) -> dict:
    ...
    rst_err uses protocol closed-set error codes (SPEC §1.2.8).
    Original fine-grained code preserved in rst_data.error_code.
    """
...
class CopilotCore:
    ...
    def __init__(self, config_path: str):
        ...
        # Whitelist index for cmd-type packages (ISSUE-002 hard gate)
        from whitelist_loader import WhitelistIndex
        self.whitelist = WhitelistIndex(str(self.config_dir / 'whitelists'))
    ...
    def _register_handlers(self):
        ...
        print(f"[copilot] Registered {len(self._handlers)} handlers "
              f"({_registered_from_directive} from @directive auto-discovery, "
              f"{_skill_count} from skill bridge), "
              f"{len(self._alias_map)} alias mappings, "
              f"{len(self._security_overrides)} security overrides")
    ...
    def dispatch(self, parsed: dict) -> dict:
        ...
        # ── Whitelist gate (ISSUE-002): cmd-type instructions must be registered ──
        wl_entry = self.whitelist.lookup(parsed['domain'], parsed['action'])
        if wl_entry is not None:
            # Build arg string from params for regex matching
            arg_str = " ".join(parsed['params']) if parsed['params'] else ""
            if wl_entry.get("_compiled") and not wl_entry["_compiled"].fullmatch(arg_str):
                self._error_count += 1
                return error('ACCESS_DENIED',
                            f'参数不符白名单: {lookup} args={arg_str!r}')
