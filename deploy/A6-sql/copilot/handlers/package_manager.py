...
class PackageManagerHandlers:
    ...
    def _write_package_ops(self, pkg_id: str, schema: dict) -> int:
        ...
        for d in directives:
            ...
            operations[op_id] = {
                ...
                "returns": "rst_data = handler output dict",
            }
            written += 1
    ...
    def _invalidate_package(self, pkg_id: str):
        """Invalidate an installed package: clean dynamic bindings, routes, and sys.modules.

        Called before re-import on update (--force), and after file removal on uninstall.
        """
        import sys

        # 1. Remove dynamically attached _handle_* methods
        safe = pkg_id.replace("-", "_")
        for attr in list(dir(self)):
            if attr.startswith(f"_handle_{safe}_"):
                try:
                    delattr(self, attr)
                    logger.debug("Removed handler attr %s for %s", attr, pkg_id)
        ...
        pkg_prefix = f"packages.{pkg_id}."
        for mod_name in list(sys.modules.keys()):
            if mod_name == f"packages.{pkg_id}" or mod_name.startswith(pkg_prefix):
                del sys.modules[mod_name]
                logger.debug("Purged module from sys.modules: %s", mod_name)

    def _load_and_wire(self, pkg_id: str, src_dir: pathlib.Path | None = None):
        """Import a package handler fresh and wire handlers. No reload.
    ...
    def _handle_text_cli_co_install(self, params: list) -> dict:
        ...
        # ── Load handlers (import_module, no reload) ──
        # For --force: invalidate old registrations first, then fresh import.
        try:
            if force:
                self._invalidate_package(pkg_id)
            handler_wired = self._load_and_wire(pkg_id, src_dir)
            logger.info("co-install %s: handlers loaded (ops=%d, wired=%s)",
                        pkg_id, ops_written, handler_wired)
        except Exception as e:
            logger.warning("co-install %s: import failed, will need restart: %s", pkg_id, e)

        # Refresh whitelist index so newly installed cmd packages are recognized
        try:
            self.whitelist.refresh()
        except Exception as e:
            logger.debug("co-install %s: whitelist refresh skipped: %s", pkg_id, e)
    ...
    def _handle_text_cli_co_uninstall(self, params: list) -> dict:
        ...
        # ── Invalidate package (dynamic bindings + sys.modules) ──
        self._invalidate_package(name)

        # ── Re-register (reload not needed since module is already gone) ──
        self._register_handlers()

        # Refresh whitelist index so removed cmd packages are recognized
        try:
            self.whitelist.refresh()
        except Exception as e:
            logger.debug("co-uninstall %s: whitelist refresh skipped: %s", name, e)
