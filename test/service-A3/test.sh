...
#
# Three modes, distinguished by env var:
#
...
#
#   Mode 2 — layer test:
#     Auto-start deploy/A3-service, run tests, auto-stop.
#     Injects TEXT_CLI_PACKAGE_SOURCE_DIRS → test/mock/.
#       TEXT_CLI_LAYER_TEST=A3 bash test.sh
#
#   Mode 3 — dev mode (manual):
#     Developer runs main.py from IDE / terminal, then runs tests separately.
#     No auto-start/stop.
#       bash test.sh   (same as mode 1)
# ──────────────────────────────────────────────────────────
