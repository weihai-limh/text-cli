...
Directives:
    AI;reasoning,<prompt>[,<mode>]       → text inference
...
def _get_api_keys() -> dict[str, str]:
    ...
    for svc in _KEY_SERVICES:
        if svc not in keys:
            ...
            # Also try bare service name (for keys like ai_api_key → AI_API_KEY)
            if not env_val:
...
def _resolve_cache_refs(text: str) -> str:
    ...
    try:
        ...
        return text
    return re.sub(
        r"cache:([a-f0-9]{12,16})",
        lambda m: cache_get(m.group(1)) or f"[cache expired:{m.group(1)}]",
        text,
    )
...
def ai_text_reasoning(params: list[str]) -> str:
    ...
    if not params:
        return f"Usage: AI;reasoning,<prompt>[,<mode>]\n{_mode_help()}"
    ...
    if not api_keys:
        return "AI keys not configured. Register: key;register,ai_api_key,<key>,api_key"
