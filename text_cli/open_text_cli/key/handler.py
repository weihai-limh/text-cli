...
if SQLITE_ENABLED:
    ...
    def key_register(params: list[str]) -> str:
        ...
        Single credential: key;register,my_service,xxx,api_key
        Dual credential: key;register,my_service,id,secret,cloud_credentials
        """
        if len(params) < 2:
    ...
    def key_quota_track(params: list[str]) -> str:
        ...
        Set: key;quota-track,my_service,ai:infer,ai:vision
        Clear: key;quota-track,my_service
        """
        if not params:
    ...
    def key_export_xor(params: list[str]) -> str:
        """
        key;export-xor,<service>

        Read plaintext credentials from key_registry, XOR-encrypt, return hex cipher.
        Used for secure credential injection into workers/external systems.

        Example: key;export-xor,my-service → <128-char-hex>
        """
        if not params:
        ...
        # 2. Read XOR key (hyphens → underscores for env var compatibility)
        ...
        if cred_count == 1:
            ...
            # Dual credential: encrypt separately
...
def _xor_encrypt(plaintext: str, xor_secret: str) -> str:
    """Repeating-key XOR encrypt: plaintext ^ secret → hex."""
