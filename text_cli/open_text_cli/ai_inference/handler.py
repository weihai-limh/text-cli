...
def init_ai_handler(db_path: str):
    ...
    candidate = '/root/text-cli-copilot/data/key_registry.json'
...
def _get_api_keys() -> dict[str, str]:
    """Read all AI-related keys from SQLite + copilot JSON fallback."""
    keys = {}
        ...
        try:
            ...
            for svc in _KEY_SERVICES:
                ...
                if entry and 'encrypted_value' in entry:
                    ...
                    if secret:
                        # XOR decrypt (inline to avoid cross-module import)
...
def ai_text_reasoning(params: list[str]) -> str:
    ...
    prompt supports cache:<key> → auto-fetch cached text substitution.
    Modes: auto/fast/quality/model_name[,cache] — ,cache caches output.
    """
    if not AI_ENABLED:
        return 'AI module not installed'

    if not params:
        return f'Missing params: AI;reasoning,<prompt>[,mode]\n{_mode_help()}'

    prompt = params[0]
    mode = params[1] if len(params) > 1 else 'auto'

    # Parse mode: bare mode name vs comma-separated; also check trailing params
    want_cache = False
    if ',' in mode:
        parts = mode.split(',')
        mode = parts[0]
    ...
    # cache:<key> in prompt → substitute cached text
    ...
    if not api_keys:
        return 'AI keys not configured. Register first: key;register,zhipu,<key>,api_key etc.'
...
def ai_vision_reasoning(params: list[str]) -> str:
    ...
    if len(params) < 2:
        return (
            f'Missing params: AI;vision,<prompt>,<image>[,mode]\n'
            f'Image supports: http/https URL, base64 data URI, or cache:<key>\n'
            f'{_mode_help()}'
        )
    ...
    try:
        ...
        return f'Vision inference exception: {e}'
