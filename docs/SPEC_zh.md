...
尾参可附加语言覆盖：`,zh` / `,en`（仅影响文本/极简模式；JSON 模式返回全部 locale 变体，见 §7.2）。
...
对于非聚合域的普通指令（含直接调用 quota;check 自身），返回 status:"stop" 仅作为信号原样透传给调用方，不触发任何自动降级——因为非聚合域不存在"可切换的下一提供方"。调用方收到 stop 后，若需故障转移，应由消费侧按 §5.4（聚合降级链）或业务层自行决策。
