...
> ⚠️ **主锚点已升级**：自 TCC Re-genesis（D6）起，铸造主锚从 `p_text-cli.md`（chat-hash）升级为 `p_git-cli.md`（git 交付事件）。本文档的双文件架构设计原理（来源账本 ≠ 结果账本）保持不变，但锚点引用已更新。
>
> **文档类型**：技术方案
> **版本**：v2.0 | **日期**：2026-05-03（原始）/ 2026-07-24（锚点更新）
> **适用范围**：text-cli 内部
> **关联文档**：`tcc-regenesis-plan.md`（D6 重新创世策划）、`tcc-internal-metering.md`（D3 TCC 内部定位）
