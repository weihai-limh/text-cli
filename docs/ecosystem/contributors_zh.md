...
> **关联文档**：`charter_zh.md`（生态宪章 §三）、`collaboration_zh.md`（项目协作规范）
...
- **Tide 🌊（Agent 端 / DeepSeek）** — 协议进化（SPEC v1.0→v1.2）、聚合指令体系设计与实现、nocode 指令包机制验证、handler_inits 自动注册、manifest 包生命周期、service_manifest 白名单、管道闭包原则确立、收敛模板机制、tc-markdown/tx-cloud/bd-cloud 指令包、quota-manage amount 扩展、task-manager tracked 模式、skill_bridge output_adapter、文档体系 v1.2（6 新 + 6 废）、README v2 重写、hero fragments 蒸馏系统、协议安全审计、生态推演与压力测试、宪章审读与权益提案、异步通信机制设计、GitHub 集成与自动化、OpenClaw 部署与配置、Agent 指令工具包架构设计与实现、分布式存续网络建立、铸造信源双文件架构提案、项目 README 架构更新、SQLite 模块体系、AI辅助模块、语义嵌入服务、防篡改护卫、文贝经济体系文档
...
| Governance / 治理机制  | 文贝第三期分配 / TCC Distribution #3 (45 TCC) | lemondy & Tide 🌊 (Agent) | 按实际活跃度分配 + 铸造池预留 / Activity-based + minting pool reserve |
| Core Code | 聚合指令系统 / Aggregate Instruction System | Tide 🌊 (Agent) | 域级降级链：多提供方收敛为单一入口 |
| Core Code | handler_inits 自动注册 / handler_inits Auto-Registration | Tide 🌊 (Agent) | 34 try 块缩减为循环：install 写/uninstall 删/重启自加载 |
| Core Code | manifest 包生命周期 / Manifest Package Lifecycle | Tide 🌊 (Agent) | install/export/uninstall 全流程追踪 |
| Core Code | service_manifest 白名单 / service_manifest Whitelist | Tide 🌊 (Agent) | 从黑名单到白名单：外部只看聚合入口 |
| Core Code | tc-markdown 指令包 / tc-markdown Package | Tide 🌊 (Agent) | Markdown 文件读取/标题提取/章节切分 |
| Core Code | tx-cloud 指令包 / tx-cloud Package (5 directives) | Tide 🌊 (Agent) | 翻译/ASR/OCR/扫码，异步两态模型 |
| Core Code | bd-cloud 指令包 / bd-cloud Package (5 directives) | Tide 🌊 (Agent) | 百科/搜索/OCR/视频笔记，双鉴权+OAuth |
| Core Code | quota-manage amount 扩展 / quota amount extension | Tide 🌊 (Agent) | 从调用次数到用量维度 |
| Core Code | task-manager tracked 模式 / task-manager tracked mode | Tide 🌊 (Agent) | 外部异步任务实时查询，不做后台轮询 |
| Core Code | skill_bridge output_adapter | Tide 🌊 (Agent) | Provider 专用适配器：字段映射到规范格式 |
| Documentation / 文档体系  | README v2 — 从协议到技能交付服务 | Tide 🌊 (Agent) & lemondy | 七章叙弧 + 三人同构 + AI成长弧线 + 渐进接入表 |
| Documentation / 文档体系  | docs/CN v1.2 — 6 份新指南 + schema 审计 | Tide 🌊 (Agent) | 开发/非开发/架构/path+aggregate/skill+MCP 全覆盖 |
| Documentation / 文档体系  | 英雄碎片蒸馏系统 / Hero Fragments Distillation | Tide 🌊 (Agent) | 每日关键决策+教训编码，11 条新教训（#27-#37） |
